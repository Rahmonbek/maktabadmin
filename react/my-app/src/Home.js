...
const Home = ({ routes }) => {
	...
	const [styleMenu,setStyle]=useState('250px')
	const menu = [
		{   id:1,
		...
		{   id:5,  
			...
			name: "Sport mashg'ulotlari",
			icon:<BiBasketball/>
		},
		{   id:6,  
			path: "/home/musiqatogaraklari/uz",
			name: "Musiqa to'garaklari",
			icon:<FiMusic/>
		},
		{   id:7,  
	...
	return (
		...
		<Container fluid style={{padding:'0',position:'relative'}}>
			...
			<Col lg={12} style={{position:'sticky'}}>
				...
				<div className={styles.topMenu}>
					<h1>Star admin</h1>
					<span className={styles.crown}><FaCrown style={{fontSize:'30px'}}/></span>
				</div>
				<Menu
				style={{overflowY:'scroll',height:'90%',overflowX:'hidden',marginTop:'0'}}
        ...
        >
			{
				menu && isArray(menu)?menu.map(item=>{
					return(
