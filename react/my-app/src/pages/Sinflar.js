...
export default class Sinflar extends Component {
    ...
    getCurator = () => {
        getXodim().then(res=>this.setState({curators: res.data})).catch(err=>console.log(err))
    }
    getClass = () => {
        getClass().then(res=>{
    ...
    createClass = () => {
        ...
        if(this.state.editId!==null) {
            console.log(classes)
            editClass(classes, this.state.editId).then(res=>this.getClass()).catch(err=>console.log(err))
        } else {
            createClass(classes).then(res=>this.getClass()).catch(err=>console.log(err))
        }
        this.handleCancel()
    }
    deleteClass = (id) => {
        deleteClass(id).then(res=>this.getClass()).catch(err=>console.log(err))
    }
    selected = (value) => {
    ...
    render() {
        const columns = [
            ...
            },
            {
                ...
                key: 'key',
                render:(key)=>{
                    return(<Button type='primary' onClick={()=>{this.editClass(key-1)}}>O'zgartirish</Button>)
                }
            },
            {
                title: 'O\'chirish',
                ...
                key: 'id',
                render:(id)=>{
                    return(<Button type='danger' onClick={()=>{this.deleteClass(id)}}>O'chirish</Button>)
                }
            },
        ]
        return (
            <div>
                <Button type='primary' width='60%' onClick={this.openModal}>Sinf yaratish</Button><br/><br/>
                ...
                <Modal title="Sinf yaratish" visible={this.state.show} onCancel={this.handleCancel} footer={false}>
                    <Form>
                        <Form.Group className="mb-3" controlId="curator">
                            ...
                            >
                                {
                                    this.state.curators!==null ? this.state.curators.map((item)=>{
                                        console.log(item.id)
                        ...
                        <Button type='danger' htmlType='button' style={{marginRight: '10px'}} onClick={this.handleCancel}>Bekor qilish</Button>
                        <Button type='primary' htmlType='button' onClick={this.createClass}>Saqlash</Button>
                    </Form>
                </Modal>
            </div>
        )
